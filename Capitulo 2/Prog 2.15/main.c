...
/* Spa.
El programa, al recibir como datos el tipo de tratamiento, la edad y elnumero
de dias de internacion de un cliente en un spa, calcula el costo total
del tratamiento.
...
void main(void)
{
    ...
    if (COS != -1)
    {
        if (EDA >= 60)
            ...
            if (EDA <= 25)
            COS = COS * 0.85;
        printf("\nClave de tratamiento: %d\t Dias: %d\t Costo total: %8.2f", TRA,DIA,COS);
